...
docker exec nexus-postgres pg_isready -U "${PG_ADMIN_USER}" &>/dev/null \
  || { echo "✗ Postgres no está corriendo. Ejecuta 03-deploy-postgres-first.sh"; exit 1; }

PG_CMD="docker exec -e PGPASSWORD=${PG_ADMIN_PASS} nexus-postgres psql -U ${PG_ADMIN_USER} -d postgres"
