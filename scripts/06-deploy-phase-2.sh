...
docker ps --filter "name=nexus-n8n-main" --filter "status=running" --format "{{.Names}}" \
  | grep -q "nexus-n8n-main" || { echo "✗ Phase 1 no está corriendo. Ejecuta 05-deploy-phase-1.sh primero"; exit 1; }
